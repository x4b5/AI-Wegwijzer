    ...
    options.forEach((el) => {
      ...
      if (el.tagName.toLowerCase() !== 'button') {
        ...
        el.addEventListener('keydown', (ev) => {
          ...
          if (key === 'Enter' || key === ' ') {
            ev.preventDefault();
            const id = (el.getAttribute('aria-controls') || '').replace(
              /^answer-/,
              ''
            );
...
function updateLastModifiedDate() {
  ...
  if (lastUpdatedElement) {
    ...
    if (
      lastUpdatedElement.tagName &&
      lastUpdatedElement.tagName.toLowerCase() === 'time'
    ) {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  (function initDockVisibilityToggle() {
    try {
      ...
      if (!btn) {
        ...
        btn.setAttribute(
          'aria-label',
          savedHidden ? 'Dock tonen' : 'Dock verbergen'
        );
        btn.setAttribute(
          'title',
          savedHidden ? 'Dock tonen' : 'Dock verbergen'
      ...
      btn.addEventListener('click', function () {
        ...
        btn.setAttribute(
          'aria-label',
          hidden ? 'Dock tonen' : 'Dock verbergen'
        );
    ...
    function (ev) {
      try {
        ...
        const toggles = Array.from(
          document.querySelectorAll('.sidebar-toggle-btn')
        );
  ...
  if (btnMoveLeft) {
    btnMoveLeft.addEventListener('click', function () {
      moveWindowLeft();
      if (typeof LearningProgress !== 'undefined')
        LearningProgress.recordTask('moveLeft');
    });
  }
  ...
  if (btnOpenLumo) {
    btnOpenLumo.addEventListener('click', function () {
      openLumoPopup();
      if (typeof LearningProgress !== 'undefined')
        LearningProgress.recordTask('openedLumo');
    });
  }
  ...
  if (btnOpenPerplexity) {
    btnOpenPerplexity.addEventListener('click', function () {
      openPerplexityPopup();
      if (typeof LearningProgress !== 'undefined')
        LearningProgress.recordTask('openedPerplexity');
    });
  }
...
const LearningProgress = (function () {
    ...
    if (!el) {
      ...
      reset.addEventListener('click', function () {
        const ok = window.confirm(
          'Weet je zeker dat je alle quiz- en taakvoortgang wilt wissen?'
        );
        if (!ok) return;
        try {
  ...
  function calcProgress() {
    ...
    const done = targets.filter(
      (id) => state.quizzes[id] && state.quizzes[id].correct
    ).length;
